...
class MatchIdentityDomainTest {
    @Test
    @DisplayName("Verificar se exceção é lançada quando o valor é null")
    void testMatchIdentityDomainWithNullValue() {
        assertThrows(
                NullPointerException.class,
                () -> new MatchIdentityDomain(null),
    ...
    void testMatchIdentityDomainWithEmptyValue() {
        assertThrows(
                IllegalArgumentException.class,
                () -> new MatchIdentityDomain(""),
                "Esperado lançamento de exceção quando valor é vazio"
        );
    }

    @Test
    @DisplayName("Verificar geração de MatchIdentityDomain aleatório")
    void testRandomMatchIdentityDomainGeneration() {
        MatchIdentityDomain randomId = MatchIdentityDomain.generateRandomPart();
        ...
        assertTrue(randomId.value().startsWith(MatchIdentityDomain.MATCH_FLAG),
                "O valor do random ID deve começar com o prefixo definido");

        assertEquals(MatchIdentityDomain.MATCH_FLAG.length() + MatchIdentityDomain.LENGTH_OF_RANDOM_PART,
                randomId.value().length(),
                "O comprimento do ID deve ser igual ao comprimento do prefixo mais o comprimento da parte aleatória");

        assertTrue(randomId.value().substring(MatchIdentityDomain.MATCH_FLAG.length()).chars()
                        .allMatch(c -> MatchIdentityDomain.ALPHABET.indexOf(c) != -1),
